...
class LevitationBase(gym.Env):
    def __init__(self, fs, fs_ctrl, cascade=True):
        ...
        if self.cascade:
            ...
            state_min, state_max = np.array([0.0, -np.inf]), np.array([0.014, 1.0])
            sens_min, sens_max = np.array([0.0, -np.inf]), np.array([0.014, 1.0])
            obs_min, obs_max = np.array([0.0, -np.inf]), np.array([0.014, 1.0])
        else:
            act_max = np.array([24.0])
            state_min, state_max = np.array([0.0, -np.inf, 0.0, -1e3]), np.array([0.014, 1.0, 3.0, 1e3])
            sens_min, sens_max = np.array([0.0, -np.inf, 0.0, -1e3]), np.array([0.014, 1.0, 3.0, 1e3])
            obs_min, obs_max = np.array([0.0, -np.inf, 0.0, -1e3]), np.array([0.014, 1.0, 3.0, 1e3])
        ...
        if self.cascade:
            ...
            self.observation_space = LabeledBox(
                labels=('xb', 'dxb'),
                low=obs_min, high=obs_max, dtype=np.float32)
            self.action_space = LabeledBox(labels=('ic',),
                low=-act_max, high=act_max, dtype=np.float32)
        else:
            self.state_space = LabeledBox(
                labels=('xb', 'dxb', 'ic', 'icd'),
                low=sens_min, high=state_max, dtype=np.float32)
            self.sensor_space = LabeledBox(
                labels=('xb', 'dxb', 'ic', 'icd'),
                low=sens_min, high=sens_max, dtype=np.float32)
            self.observation_space = LabeledBox(
                labels=('xb', 'dxb', 'ic', 'icd'),
                low=obs_min, high=obs_max, dtype=np.float32)
            self.action_space = LabeledBox(labels=('v',),
                low=-act_max, high=act_max, dtype=np.float32)

        self.reward_range = (0.0, self.timing.dt_ctrl)

        self.xb0 = np.array([0.014]) # operating point
        self.xbg = np.array([0.005]) # goal gap
    ...
    def _ctrl_step(self, a):
        ...
        for _ in range(self.timing.n_sim_per_ctrl):
            a_cmd = np.clip(a, self.action_space.low, self.action_space.high)
            s = self._sim_step(s, a_cmd, self.cascade)
            s = np.clip(s, self.state_space.low, self.state_space.high)
        return s, a_cmd  # return the last applied (clipped) command

    def _rwd(self, s, a):
        cost = (1e3 * (s[0] - self.xbg))**2 + 1e-3 * a[0] ** 2
