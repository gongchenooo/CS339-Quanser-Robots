...
class MPC(object):
    ...
    def act(self, state, dynamic_model):
        ...
        # optimizer = Hive.BeeHive(lower=[float(self.action_low)]*self.horizon, upper=[float(self.action_high)]*self.horizon,fun=self.evaluator.evaluate,numb_bees=self.numb_bees,
        #                          max_itrs=self.max_itrs,verbose=False)
        optimizer = Cannon(lower=[float(self.action_low)] * self.horizon,
